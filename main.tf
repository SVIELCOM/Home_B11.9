...
data "template_file" "inventory" {
  ...
  vars = {
    user  = "ubuntu"
    host1 = join("", [yandex_compute_instance.server1.network_interface[0].nat_ip_address])
    host2 = join("", [yandex_compute_instance.server2.network_interface[0].nat_ip_address])
  }
}
